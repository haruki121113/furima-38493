  ...
  describe '商品出品' do
    ...
    context '商品出品できないとき' do
      ...
      it '価格が300円未満では出品できない' do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include("Merchandise price must be greater than or equal to 300")
      end
      it '価格が9999999円を超えると出品できない' do
        @item.merchandise_price = 10000000
        @item.valid?
        expect(@item.errors.full_messages).to include("Merchandise price must be less than or equal to 9999999")
      end
